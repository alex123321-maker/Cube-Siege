...
- **`ChunkBuilder` (`scripts/world/chunk_builder.gd`) и `MapGenerator` (`scripts/map_generator.gd`)**:
  ...
  - Единый `ArrayMesh` с 4 поверхностными проходами (Лес, Равнина, Горы, Скалы) и `ConcavePolygonShape3D` на чанк: минимальный оверхед по узлам (49 StaticBody3D в начальной области 7x7 при `load_radius=3`; при прямолинейном движении рабочий набор стабилизируется на 63 чанках ($9 \times 7$), при диагональном — до 81 чанка ($9 \times 9$), а гистерезис выгрузки `unload_radius=5` строго ограничивает теоретический максимум величиной $\le 121$ чанк ($11 \times 11$), сохраняя O(1) профиль памяти).
...
### 4.4. Назначение физических слоёв (Physics Collision Layers)
В проекте действует строгое разделение 3D-коллизий (слои 1..6, сконфигурированы в `project.godot`):

| Слой | Маска (Бит) | Название | Назначение | Участники |
| :---: | :---: | :--- | :--- | :--- |
| **1** | `1` | `world` | Твёрдый ландшафт и непроходимые препятствия | Воксельный рельеф, стоящие деревья, целые камни/железо, стены баз |
| **2** | `2` | `player` | Твёрдые тела игрока и союзных мишеней | Игрок (`player.tscn`), Манекен (`decoy_dummy.tscn`) |
| **3** | `4` | `enemies` | Твёрдые тела противников | Рядовые мобы, стрелки, таранщики, боссы |
| **4** | `8` | `combat` | Боевой обмен уроном (Хитбоксы и Хёртбоксы) | `HurtboxArea` деревьев/камней/врагов/игрока, снаряды стрел, взмахи меча |
| **5** | `16` | `canopy_occlusion` | Детекция перекрытия камеры кронами | `CanopyOcclusionArea` взрослых деревьев для полупрозрачности |
| **6** | `32` | `interaction` | Выделенный слой зон взаимодействия | `InteractionZone` (все интерактивные объекты: срубленные ресурсы, постройки, верстак, портал, пикапы) |

### 4.5. Архитектура взаимодействия (InteractionZone & Safe Harvest)
- **Изоляция взаимодействия от боя и геометрии**: Сенсор игрока `InteractionSensor` (`scenes/player.tscn`) сканирует исключительно Layer 6 (`collision_mask = 32`). Ни боевые снаряды, ни твёрдые тела персонажа не взаимодействуют со слоем 6.
- **`InteractionZone` (`scripts/interaction/interaction_zone.gd`)**: Специализированный компонент `Area3D` на слое 32 (`monitoring = false`, `monitorable = true`), хранящий типизированную ссылку `target_node`. Присутствует на всех типах интерактивных сущностей (`ResourceTree`, `ResourceRock`, `FreeResourcePickup`, `BuildingBase`, `Workbench`, `PortalController`, `RelicPedestal`).
- **Снимок физики как единый источник правды**: `PlayerInteraction` опрашивает `sensor.get_overlapping_areas()` каждый кадр, не полагаясь на уязвимые сигналы `body_entered`/`body_exited`. При разрушении объекта отключение твёрдой коллизии (слой 1) и хёртбокса (слой 4) не инвалидирует нахождение в `InteractionZone` (слой 6) — цель доступна сразу без необходимости отходить и возвращаться.
- **Дедупликация и защита фокуса**: Несколько зон одного объекта дедуплицируются; удерживаемый таймер сбрасывается только при смене цели или отпускании клавиши [E]; удаление/выгрузка объекта грациозно очищает фокус и таймер без висячих ссылок.
- **Безопасный контракт сбора (Safe Harvest)**: Метод `harvest(player)` валидирует получателя (`BuildingSystem`) до выполнения необратимых изменений. Если получатель отсутствует, сбор возвращает `false`, объект остаётся интерактивным, всплывающие окна и запись в `MapGenerator.record_harvest()` не производятся. Защита от повторного входа исключает дублирование наград.
