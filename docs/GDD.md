...
- `[W][A][S][D]`: Направленное перемещение персонажа относительно вектора прицеливания (`AimDirection`) на плоскости XZ: `[W]` — вперёд по линии прицеливания, `[S]` — назад, `[A]` / `[D]` — боковой стрейф влево / вправо. При наличии принудительной цели (`forced_target`) базис ориентируется на вектор к цели (`[W]` к цели, `[S]` от цели, `[A]`/`[D]` стрейф по орбите вокруг неё).
- `[Мышь Move]`: Вектор прицеливания (`AimDirection`) и поворот торса/оружия; задаёт ориентацию базиса WASD (если не задан `forced_target`).
