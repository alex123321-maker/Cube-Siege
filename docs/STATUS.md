...
| **Main Scene (Главный игровой цикл)** | `scenes/main.tscn`<br>`scripts/main.gd` | Корневой координатор игры: загрузка карты, инициализация игрока, портала, камеры, HUD и обработка исходов (победа/поражение, прослушивание `EventBus.portal_evacuated`). |
| **Player Controller (Архитектура подсистем)** | `scenes/player.tscn`<br>`scripts/player_prototype.gd`<br>`scripts/player/` | Игрок (`CharacterBody3D`), декомпозированный на 8 специализированных подсистем: `PlayerMovement`, `PlayerAim`, `PlayerHealth`, `PlayerProgression`, `PlayerInteraction`, `PlayerPresentation`, `PlayerCombat`, `PlayerAbilities` и типизированный контракт `InteractableTarget`. Полностью исключены глобальные root `find_child` поиски, сохранена 100% обратная совместимость API. |
| **Class Mechanics (Воин / Лучник / Инженер)** | `scripts/player/player_combat.gd`<br>`scripts/player/player_abilities.gd`<br>`scripts/player_prototype.gd` | В коде реализованы раздельные наборы атак, умений и ультимейтов для 3 классов через `PlayerCombat` и `PlayerAbilities`:<br>• **Воин**: взмах мечом (`trigger_slash`), круговой спецвзмах [ПКМ], блок щитом [Q], ультимейт «Дуэль чести» [F] с привязкой цепью (`duel_tether.tscn`) и +20% урона.<br>• **Лучник**: выстрел стрелой (`trigger_arrow_shot`), пробивающая стрела сквозь 6 целей [ПКМ], приманка-чучело [Q], ультимейт «Орлиный глаз» [F] (отдаление камеры `camera.size = 34.0`; бонус к дальности в коде не подключен).<br>• **Инженер**: удар молотом (`trigger_hammer_smash` с ремонтом построек), установка турели [ПКМ], дистанционная мина [Q], ультимейт «Тактический ядерный удар» [F] (AoE 10м, 1.2с телеграф, 300 урона). |
| **Portal Controller & Extraction** | `scenes/portal.tscn`<br>`scripts/portal_controller.gd` | Центральный портал: зоны взаимодействия, 3 стадии починки ресурсами (дерево, камень), обратный отсчёт эвакуации (45 сек), эмит события победы через `EventBus.portal_evacuated` без глобальных root `find_child` поисков. |
| **Day/Night Cycle** | `scripts/day_night_cycle.gd` | Таймер смены дня и ночи, интерполяция направленного света (`DirectionalLight3D`), цвета тумана и запуск сигналов `day_started`/`night_started`/`cycle_time_updated`. Полная изоляция от UI: не содержит ссылок на HUD/Control узлы и не мутирует их напрямую. |
...
| **Resource Nodes & Pickups** | `scenes/resource_tree.tscn`<br>`scenes/resource_stone.tscn`<br>`scenes/resource_iron.tscn`<br>`scripts/world/free_resource_pickup.gd` | Полнотелые интерактивные объекты ресурсов с запасом прочности, отдачей при ударе, стадиями разрушения, вариациями дубов и тирами камня/железа + свободные собираемые пикапы (1-3 ед., без блокировки перемещения) и прозрачность крон деревьев при перекрытии камеры. |
...
| **UI: Workbench Modal** | `scenes/workbench_modal.tscn`<br>`scripts/workbench_modal.gd` | Модальное окно крафта экипировки и мастерства на верстаке; взаимодействие с `BuildingSystem` через явную типизированную зависимость без root `find_child`. |
...
| **GUT Testing Suite** | `addons/gut/`<br>`.gutconfig.json`<br>`tests/` | 50+ автоматических тестов (smoke, unit, integration), полное покрытие безопасных зон, кошелька, персистентности, 8 подсистем игрока, таранщика, директора волн, нативного пробника и типизированных сценариев взаимодействия. |
...
## ⚠️ CURRENT TECH DEBT & STABILIZATION (Аудит и долг стабилизации — Issue #20)
...
1. **P0: Сломанный подбор срубленных ресурсов (`ResourceTree`, `ResourceRock`)**:
   - `fell_tree()` отключает коллизию `CollisionShape3D`, вызывая `body_exited` на `InteractionSensor` игрока и стирая ноду из кандидатов. Требуется замена на мгновенный спавн магнитных лут-дропов.
...
3. **P0: Стрельба с высоты пролетает над врагами**:
   - Вектор прицеливания мыши обнуляет Y-составляющую (`diff.y = 0.0`), а `TerrainCombatRules.update_projectile_height` принудительно держит стрелу горизонтально в воздухе (`new_y = current_pos.y`) при `currently_over_drop`. Требуется 3D-вектор полета.
4. **P1: Зебра-лестницы гор и хаос рельефа**:
   - Градиент 0.7 + высокочастотный симплекс-шум + квантование порождают ступенчатую зебру через каждый метр. Требуется террасированный профиль (плато 8–16м, уступы 2–3 блока).
5. **P1: Полное отсутствие звука (гробовая тишина)**:
   - В проекте нет аудиофайлов. Требуется базовый пакет SFX (12 звуков для ударов, шагов, выстрелов, эмбиента).
...
7. **P1: Инстанцирование сотен нод на чанк (1800+ нод на карте)**:
   - Сеточный белый шум (16% на клетку) спавнит индивидуальные сцены вместо кластеров и `MultiMeshInstance3D` для декораций.
