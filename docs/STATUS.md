...
| **MasteryManager (Мета-прогресс)** | `scripts/mastery_manager.gd` | Autoload-синглтон. Управляет боевым опытом, уровнями и 4 ветками талантов (Кровожадность, Выживаемость, Проворство, Ремесло). Сохраняет состояние в `user://mastery_save.json`. |
| **RosterManager (Ростер классов)** | `scripts/roster_manager.gd` | Autoload-синглтон. Хранит 3 класса (Воин, Лучник, Инженер), активного персонажа, распределение очков и историю забегов в `user://character_roster.json`. |
...
| **Player Controller (Прототип GDScript)** | `scenes/player.tscn`<br>`scripts/player_prototype.gd` | Игрок (`CharacterBody3D`): изометрическое движение (WASD), чтение активного класса из `RosterManager`, прицеливание (мышь / стрелки клавиатуры), рывок уклонения (Space), стрелка компаса к порталу, инвентарь ресурсов и получение урона. |
...
| **Safe Zone Detector (Замкнутый контур)** | `scripts/safe_zone_detector.gd` | Алгоритм flood-fill: находит клетки сетки внутри замкнутого периметра стен (минимум 4 стены) и передаёт их в `WaveDirector.set_safe_zone_cells()`, блокируя появление мобов внутри базы. Проверка дистанции до портала и урон тьмы игроку отсутствуют. |
| **Map Generator (Процедурная карта)** | `scenes/map_generator.tscn`<br>`scripts/map_generator.gd` | Генерация сетки 65x65 клеток (`map_radius = 32`, координаты от -32 до 32), спавн деревьев, камней и железа через FastNoiseLite с зоной клиринга портала (радиус 6.5м). |
...
| **Building System (Строительство)** | `scripts/building_system.gd`<br>`scripts/building_base.gd` | Размещение построек по сетке с проверкой стоимости ресурсов, превью-сеткой и вызовом радиального меню. |
...
| :--- | :--- | :--- | :--- |
| **Player Controller (C++ Graybox)** | `src/player_controller.cpp`<br>`src/player_controller.h`<br>`bin/cube_siege.gdextension` | Нативный класс `PlayerController` скомпилирован в GDExtension DLL/.so для валидации сборки. | Не используется в активной игровой сцене `player.tscn` (сцена подключена к `scripts/player_prototype.gd`). |
| **Enemy AI & Wave Director** | `scripts/wave_director.gd`<br>`scripts/enemy_base.gd`<br>`scenes/enemy_dummy.tscn`<br>`scenes/enemies/ranged_skirmisher.tscn`<br>`scenes/enemies/siege_breaker.tscn`<br>`scenes/enemies/boss_gorgon.tscn` | Директор ночных волн: интервал спавна 1.6с, кольцо спавна радиусом 20..28м от игрока, отклонение спавна внутри SafeZone. Пул: максимум 20 мобов при обычной волне, 4 моба при активном боссе. 4 типа врагов: Dummy, Skirmisher, Siege Breaker, Boss Gorgon. | Враги двигаются по прямой линии к цели `(target - pos).normalized()`. Отсутствует волновое поле (Flowfield) и алгоритмы разделения толпы (crowd steering/boids). |
...
1. **Тройное владение состоянием сохранений (Persistence Ownership)**:
   - `SaveManager` (`scripts/save_manager.gd`) сохраняет `meta_xp`, `survived_runs`, `heroes_roster`, `unlocked_classes` в `user://cube_siege_save.json`.
   - `RosterManager` (`scripts/roster_manager.gd`) сохраняет слоты классов, уровни, боевой опыт и таланты в `user://character_roster.json`. При смерти персонажа `RosterManager` сбрасывает его параметры к 1 уровню, тогда как `SaveManager` удаляет героя из списка.
   - `MasteryManager` (`scripts/mastery_manager.gd`) сохраняет общий боевой опыт и очки талантов в `user://mastery_save.json`.
   - *План погашения*: объединение под единый фасад управления сохранениями (`SaveManager`) с версионированием схемы данных.

2. **Несоответствие параметров ультимейта Лучника**:
...
3. **Логика лимитов WaveDirector при боссах**:
   - Формула `var effective_max: int = 4 if not bosses.is_empty() else max_concurrent_enemies` ограничивает число обычных врагов 4 мобами при живом боссе и 20 мобами в обычной волне. Запланированное расписание волн 10/20/30 в коде не реализовано.
