...
func _run() -> void:
	hero = _arg("--hero", "warrior")
	class_id = ["warrior", "archer", "engineer"].find(hero)
	...
	if mode == "gameplay":
		...
		camera.make_current()
		model = player.get_node("Visuals/Hero" + hero.capitalize())
		await _frames(30)
	else:
		_setup_studio()
		await _frames(3)
...
func _setup_studio() -> void:
	...
	model = load("res://assets/models/characters/hero_%s.tscn" % hero).instantiate()
...
func _studio_stills() -> void:
	...
	camera.position = Vector3(3.6,2.6,-6)
	camera.look_at(Vector3(0,1,0))
	for item: Array in [["run", "walk", .12], ["lmb_contact", "attack", .06], ["rmb_contact", "special", .15], ["utility", "utility", .15], ["ultimate", "ultimate", .3]]:
		...
		await _capture(item[0]+".png")
	if hero == "archer":
		for entry: Array in [["draw", "attack", .055], ["release", "attack", .10], ["charge", "special", .25], ["charged_release", "special", .31]]:
			ap.play(entry[1])
			ap.seek(entry[2],true)
			ap.pause()
			await _capture(entry[0]+".png")
	if hero == "engineer":
		for t: float in [.14,.15,.16]:
			ap.play("special")
			ap.seek(t,true)
			ap.pause()
			await _capture("placement_%03d.png" % int(t*1000))
	...
	if not before_path.is_empty():
		...
		before.get_node("AnimationPlayer").play("idle")
		before.get_node("AnimationPlayer").advance(0)
		before.get_node("AnimationPlayer").pause()
		...
		for i: int in range(2):
			...
			caption.text = "BEFORE / repository HEAD" if i == 0 else "AFTER / Blender candidate"
	...
	if hero == "warrior":
		for entry: Array in [["idle", .1], ["walk", .12], ["attack", .06], ["special", .15], ["block", .1], ["ultimate", .3]]:
			ap.play(entry[0])
			ap.seek(entry[1], true)
			ap.advance(0)
			ap.pause()
			var grip: Node3D = model.find_child("SwordGrip", true, false)
			var target: Vector3 = grip.global_position
			camera.position = target + Vector3(2, 1.0, -2.2)
			camera.size = 1.2
			camera.look_at(target)
			await _capture("sword_grip_" + entry[0] + ".png")
	camera.position = Vector3(3.6,2.6,-6)
	camera.size = 3.8
	camera.look_at(Vector3(0,1.3,0))
...
func _observe(action: String, expected: String) -> void:
	...
	print("HERO_REVIEW=",JSON.stringify(row))
