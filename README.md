...
С наступлением ночи освещение сменяется холодным лунным светом, а режиссер волн направляет орды врагов на штурм базы и портала. В кульминационные ночи пробуждаются колоссальные боссы — такие как **Gorgon the Trampler**!
...
| **Инженер (Engineer)** | Контроль и оборона | Гаечный ключ / Молот | Переносная турель + Дистанционная мина | **Тактический ядерный удар (Tactical Nuke)**: орбитальный ракетный залп по площади 10м с телеграфом 1.2с и уроном 300 HP |
...
- **GDScript Gameplay Core**: стейт-машины, процедурная карта (`map_generator.gd`), смена дня/ночи (`day_night_cycle.gd`), верстак, строительство и UI.
- **C++ GDExtension**:
  - *[CURRENT]* Graybox контроллер игрока (`src/player_controller.cpp`) и обвязка сборки SCons.
  - *[PLANNED]* Векторное поле навигации 2D Flowfield Pathfinding для 500+ мобов.
- **Навигация толпы (Crowd Navigation)**:
  - *[CURRENT]* Прямое векторное преследование цели с преодолением воксельных ступеней (`scripts/enemy_base.gd`).
  - *[PLANNED]* 2D Flowfield на основе волнового фронта Дейкстры на C++ GDExtension.
- **Безопасная зона (Safe Zone)**: динамический радиус вокруг портала (`scripts/safe_zone_detector.gd`), защищающий игрока от периодического урона ночной тьмы.
- **Режиссер волн (Wave Director)**: кольцевой спавн мобов (`scripts/wave_director.gd`) с адаптивной сложностью и дросселированием свиты во время битв с боссами.
