...
static func build_chunk_terrain(
	...
	for lz in range(CHUNK_SIZE):
		for lx in range(CHUNK_SIZE):
			...
			var y_top: float = float(y) + 0.5

			# Determine top face material from primary biome
			var biome_info: Dictionary = BiomeSystem.sample_biome_weights(float(wx) + 0.5, float(wz) + 0.5, seed_val)
			var primary_biome: BiomeSystem.BiomeType = biome_info["primary"]

			var st_top: SurfaceTool = st_forest
			if primary_biome == BiomeSystem.BiomeType.PLAINS:
				st_top = st_plains
				count_plains += 1
			elif primary_biome == BiomeSystem.BiomeType.MOUNTAINS:
				st_top = st_mountains
				count_mountains += 1
			else:
				count_forest += 1
